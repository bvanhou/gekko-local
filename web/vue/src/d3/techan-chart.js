...
export default function(_data, _trades, _indicatorResults, _height, _config) {
  ...
  // get chart config from current strategy/method
    ...
    const countindicatorsExtraPlot = Object.entries(configIndicators)
      .filter(config => config[1].chart && config[1].chart.showInExtraWindow)
      .length;

    var dim = {
        width: MAX_WIDTH, height: (555 + (countindicatorsExtraPlot *105)),
    ...
    _.each(_indicatorResults, (results, name) => {
        ...
        indicatorData[name] = results.map( val => {
          let result = {date: moment.utc(val.date).toDate()};
          if (configChart && configChart.type === "stochastic"){
            ...
            result.stochasticD = !isNaN(+val.result.stochK) ? +val.result.stochK : null;
            result.stochasticK = !isNaN(+val.result.stochD) ? +val.result.stochD : null;
            ...
            result.upperBand = !isNaN(+val.result.bbandsUpper) ? +val.result.bbandsUpper : null;
             ...
             result.oversold = 30;
             result.rsi = !isNaN(+val.result) ? +val.result: null;
          }else if (!isNaN(+val.result)){
            result.value = +val.result;
          }else if (val.result !== null && typeof val.result === 'object'){
            result.value = !isNaN(+val.result.result) ? +val.result.result : null
          }else
            result.value = null;
...
// clippath
    ...
    let data = _data.map( d => {
        return {
            date: moment.utc(d.start).toDate(),
            open: +d.open,
            high: +d.high,
            low: +d.low,
            close: +d.close,
